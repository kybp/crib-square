...
(test-group "find-runs"
  (test "Returns the empty list for a hand with no runs"
        '()
        (find-runs (list (make-card 'clubs 2)
                         (make-card 'clubs 3)
                         (make-card 'clubs 5)
                         (make-card 'clubs 'king))))
  (test "A run of 3 scores a (run 3)"
        '((run 3))
        (find-runs (list (make-card 'clubs 3)
                         (make-card 'clubs 5)
                         (make-card 'clubs 9)
                         (make-card 'hearts 4))))
  ...
  (test "A run of 5 scores a (run 5)"
        '((run 5))
        (find-runs (list (make-card 'clubs 10)
                         (make-card 'clubs 'king)
                         (make-card 'clubs 'jack)
                         (make-card 'clubs 9)
                         (make-card 'clubs 'queen))))
  (test "Returns two 3-runs for a double run"
        '((run 3) (run 3))
        (find-runs (list (make-card 'clubs 'ace)
                         (make-card 'spades 2)
                         (make-card 'hearts 3)
...
(test-group "score-hand"
  (test "Four cards of the same suit scores a little-flush"
        '(little-flush)
        (score-hand (list (make-card 'clubs 2)
                          (make-card 'clubs 4)
                          (make-card 'clubs 6)
                          (make-card 'clubs 8))
                    (make-card 'spades 'king)))
  ...
  (test "A run of 3 scores a (run 3)"
        '((run 3))
        (score-hand (list (make-card 'clubs 8)
                          (make-card 'clubs 10)
                          (make-card 'clubs 9)
                          (make-card 'hearts 'ace))
                    (make-card 'clubs 2)))
  (test "A run of 4 scores a (run 4)"
        '((run 4))
        (score-hand (list (make-card 'clubs 'jack)
                          (make-card 'clubs 'king)
                          (make-card 'clubs 3)
                          (make-card 'hearts 10))
                    (make-card 'clubs 'queen)))
  ...
  (test "A pair scores a pair"
        '(pair)
        (score-hand (list (make-card 'clubs 9)
                          (make-card 'hearts 9)
                          (make-card 'spades 2)
                          (make-card 'hearts 3))
                    (make-card 'diamonds 7)))
  ...
  (test-assert "A double run scores two 3-runs and a pair"
               (let ((results (score-hand (list (make-card 'clubs 'ace)
                                                (make-card 'spades 2)
                                                (make-card 'hearts 3)
                                                (make-card 'diamonds 7))
                                          (make-card 'spades 'ace))))
                 (and (= 2 (count (lambda (x) (equal? x '(run 3))) results))
                      (= (length results) 3)
                      (memq 'pair results))))
  (test "Scores a 2-card fifteen"
        '(fifteen)
        (score-hand (list (make-card 'clubs 5)
                          (make-card 'clubs 'king)
                          (make-card 'spades 3)
                          (make-card 'spades 'ace))
                    (make-card 'hearts 8)))
  (test "Scores multiple fifteens"
        '(fifteen fifteen)
        (score-hand (list (make-card 'clubs 'king)
                          (make-card 'clubs 7)
                          (make-card 'clubs 2)
                          (make-card 'hearts 'ace))
                    (make-card 'diamonds 5))))
